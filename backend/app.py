...
app = Flask(__name__)
app.secret_key = os.getenv("FLASK_SECRET_KEY", "110512")

# DB connection configuration - Railway provides a DATABASE_URL environment variable.
...
def get_conn():
    ...
    try:
        if DB_URL:
            # If a full DATABASE_URL is provided (common on Railway), use it directly.
    ...
    if 'cart' in session and session['cart']:
        try:
            for idp, qty in session['cart'].items():
                ...
                cur.execute('UPDATE productos SET stock = stock - %s WHERE id_producto = %s', (qty, idp))
            conn.commit()
        except Exception as e:
            conn.rollback()
...
# On Railway / Gunicorn we won't use the development host. Gunicorn will import `app`.
if __name__ == '__main__':
    app.run(host='127.0.0.1', port=int(os.getenv('PORT', 5025)), debug=(os.getenv('FLASK_ENV')=='development'))
